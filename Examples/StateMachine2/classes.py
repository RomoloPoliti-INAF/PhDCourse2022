...
class Command:
    ...
    def show(self, params=None):
        print(Text(self.name, style="bold yellow", justify="center"))
        print(Text(self.description, justify="center"))
        print(f"Destination: [blue]{self.destination}[/blue]")
        tab = Table()
        ...
        if len(self.paramenters) == 0:
            print(Text("No parameters", style="bold red"))
        else:
            tb = Table()
...
class Commands:
    def __init__(self, fName: Path, verbose: bool = False):
        ...
        self.commands = []
        if verbose > 1:
            print(f"{MSG.INFO}Loading the configuration file {fName}")
...
class StaticMachine:
    ...
    def show(self):
        if self.state==STATE.IDLE:
            ...
            color="red"
        print(f"[bold green]{self.name}:[/bold green]\t[{color}]{self.state}[/{color}]")
